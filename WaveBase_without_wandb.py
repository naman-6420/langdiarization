...
from model import X_Base_E2E_LD
...
def main():
    parser = argparse.ArgumentParser(description='Training script for Base E2E model')
    ...
    parser.add_argument('--model', type=str, default='my_base_model', help='Name for the saved model file')
    ...
    model = X_Base_E2E_LD(n_lang=args.lang, feat_dim=256, n_heads=4, d_k=256, d_v=256,
                          d_ff=2048, max_seq_len=args.maxlength, device=device)
    model.to(device)

    class_weights = torch.tensor([1.7558, 0.4868, 2.6658]).to(device)
    loss_func_CRE = nn.CrossEntropyLoss(weight=class_weights, reduction='mean').to(device)
    loss_func_xv = nn.CrossEntropyLoss(weight=class_weights, reduction='mean').to(device)
